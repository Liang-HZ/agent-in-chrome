...
**How does it compare?** The table below scores only **native, out-of-the-box capability**: ✅ = works without writing code, ❌ = not available. (Most of these tools ship an arbitrary-code-execution escape hatch, so in principle anything can be rebuilt by hand — count that and every cell turns ✅ and the table stops meaning anything.) Based on a source-code read and hands-on measurement on 2026-08-18 (the Playwright MCP, browser-use, and ego lite columns re-verified on 2026-09-10 with the same task books plus targeted probes; Playwright MCP and ego lite matched agent-in-chrome / chrome-devtools-mcp on task completion, while browser-use showed real tool-surface gaps — see footnotes), against @playwright/mcp 0.0.80 · chrome-devtools-mcp 1.7.0 · browser-use 0.13.10 · ego lite 0.5.0.28. No recommendation implied — pick by your situation.
...
<sup>1</sup> `--extension` mode only (Chrome/Edge, requires installing their extension); the default mode is an isolated profile. <sup>2</sup> `--autoConnect` only (Chrome 144+, requires manually opening the gate in `chrome://inspect`, which exposes every window in that profile). <sup>3</sup> Either connect over CDP to a Chrome already started with a debug port, or use its bundled browser (isolated profile, no user login state). <sup>4</sup> A separate Chromium fork; login state comes from import rather than live borrowing — `ego-browser import` re-imports from Chrome/Edge/Brave (extensions included). <sup>5</sup> Windows supports extension mode (CLI/headless mode planned); the Linux path is written but unverified. <sup>6</sup> Playwright's `--secrets` and DevTools MCP's `--redactNetworkHeaders` are both opt-in and off by default, and the latter doesn't cover bodies. Measured: Playwright's `browser_network_request` filters Cookie and Set-Cookie out of request headers by default (full raw headers require run_code). <sup>7</sup> Solid placeholder mechanism on the input side; no redaction on network responses. <sup>8</sup> `browser_run_code_unsafe` describes itself as RCE-equivalent and is on by default. <sup>9</sup> ego has no native network helpers, but its `page.cdp()` escape hatch reaches everything (measured on 0.5.0.28): raw headers live in CDP ExtraInfo events and need manual pairing by requestId, WS frames are capturable, mocking/throttling/tracing all doable; cookies stay plaintext throughout, no redaction. <sup>10</sup> ego and browser-use both make code their primary interface (heredoc / Python) — arbitrary code execution is by design. <sup>11</sup> Measured: overlays without ARIA roles don't surface in ego's semantic snapshot (flattened into the same tree, no modality marker; only a click error names the intercepting element); its snapshot carries no element pixel coordinates, and top-level coordinates across frames must be computed by hand. browser-use filters buttons under an overlay out of its element list entirely and its coordinate clicks can falsely report success; its observation output likewise carries no pixel coordinates. <sup>12</sup> Measured: Playwright MCP 0.0.80 ships no mocking/throttling/tracing tools at all (the `browser_route`/tracing tools in its README were not released with this version), and it has no native WS frames, initiator stacks, or curl export either; all of these are reachable through the `browser_run_code_unsafe` escape hatch.
...
2. Detect the agents on your machine (Claude Code, Claude Desktop, WorkBuddy, WorkBuddy AI, ZCode, opencode, Kimi CLI, Gemini CLI, Antigravity, Qoder CLI, Qoder IDE, Trae, Codex CLI, DeepSeek Harness, plus the China builds Qoder CN CLI / Qoder CN IDE / Trae CN — 17 in total), show you the exact file and content it would write for each, and register the MCP server after you confirm (originals backed up automatically; `--yes` skips the per-agent prompts, `--agents=claude-code,trae` narrows the set, `--no-agents` skips this entirely).
